...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_babel_runtime_core_js_object_get_own_property_descriptor__ = __webpack_require__(146);
...
$export($export.S + $export.F * !__webpack_require__(46), 'Object', { defineProperty: __webpack_require__(97).f });
...
var _elementDataset = __webpack_require__(94);
...
var _Article = __webpack_require__(361);

var _Article2 = _interopRequireDefault(_Article);

var _Category = __webpack_require__(363);
...
function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; } // Modules (exports, import)
...

{
  ...
  var showHeadlines = function showHeadlines(source) {
    var newHeadlinesURL = headlinesURL + source;
    var requestHeadlines = new Request(newHeadlinesURL, { headers: headers });

    fetch(requestHeadlines).then(dataHelper.handleErrors).then(dataHelper.parseData).then(function () {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { articles: [] };
      ...
      try {
        for (var _iterator = data.articles[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var articleData = _step.value;

          if (articleData.title && articleData.description && articleData.url) {
            var article = createArticle(articleData);
  ...
  var loadSources = function loadSources(category) {
    var loadSourcesURL = void 0;
    ...
    var requestSources = new Request(loadSourcesURL, { headers: headers });

    // fetch all sources
    // arrow function
    fetch(requestSources).then(dataHelper.handleErrors).then(dataHelper.parseData).then(function () {
      var data = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : { sources: [] };
      ...
      if (typeof category === 'undefined') {
        newsCategories.insertAdjacentHTML('beforeend', createCategories((0, _Filter.categorize)(data.sources)));
      }
  ...
  var headlinesUpdate = function headlinesUpdate(event) {
    var selectedValue = event.target.value;
    showHeadlines(selectedValue);
  };

  var categoriesUpdate = function categoriesUpdate(event) {
    var categories = newsCategories.querySelectorAll('.js-category');
    if (event.target && event.target.matches('.js-category')) {
      var categoryEl = event.target;
      var categoryValue = categoryEl.dataset.value;
  ...
  var createCategories = function createCategories(categories) {
    ...
    var keys = [].concat(_toConsumableArray(categories.keys()));
    keys.unshift('all');
    ...
    return keys.map(function (key, index) {
      ...
      var category = createCategory(key);
  ...
  var createCategory = function createCategory(data) {
    return new _Category2.default(data);
  };

  var createListItem = function createListItem(source) {
    return '<option value="' + source.id + '">' + source.name + '</option>';
  };

  var createArticle = function createArticle(data) {
    return new _Article2.default(data);
  };
  ...
  var settings = _defineProperty({}, 'newsAPIURL', 'https://newsapi.org/v2');

  var apiKey = 'bdbeeb170f8c47a2b97aa0f6252bfb90';
  var headers = new Headers({
    'X-Api-Key': apiKey
  });
  ...
  var newsAPIURL = settings.newsAPIURL,
      _settings$sourcesPara = settings.sourcesParam,
      sourcesParam = _settings$sourcesPara === undefined ? 'sources=' : _settings$sourcesPara,
      _settings$categoriesP = settings.categoriesParam,
      categoriesParam = _settings$categoriesP === undefined ? 'category=' : _settings$categoriesP,
      _settings$sourcesPath = settings.sourcesPath,
      sourcesPath = _settings$sourcesPath === undefined ? '/sources' : _settings$sourcesPath,
      _settings$headlinesPa = settings.headlinesPath,
      headlinesPath = _settings$headlinesPa === undefined ? '/top-headlines' : _settings$headlinesPa;

  // template literals

  var sourcesURL = '' + newsAPIURL + sourcesPath;
  var headlinesURL = '' + newsAPIURL + headlinesPath + '?' + sourcesParam;
  var select = document.getElementById('news-channels');
...
function removeDuplicates(sources) {
  var result = sources.filter(function (item, index, array) {
...
function categorize(sources) {
  var map = new Map();
  return sources.reduce(function (map, item) {
    var category = item.category;
    if (!map.has(category)) {
      // Array.from instead of [item]
      map.set(category, Array.from(item));
    } else {
      var values = map.get(category);
...
var _loadImage = __webpack_require__(362);
...
var Article = function () {
  ...
  _createClass(Article, [{
    ...
    value: function renderImg() {
      var article = document.querySelector('article[data-url="' + this.url + '"]');
      article.insertAdjacentHTML('afterbegin', '<a class="article-link" target="_blank" href="' + this.url + '">\n        <img class="article-img" src="' + this.imageSrc + '" alt="' + this.title + '">\n      </a>');
    }
  }, {
    key: 'addImg',
    value: function addImg() {
      ...
      (0, _loadImage2.default)(this.imageSrc).then(function (image) {
        _this.renderImg();
...
function loadImage(url) {
  return new Promise(function (resolve, reject) {
    ...
    image.onerror = function (error) {
      var msg = "Couldn\u2019t load image from " + url;
